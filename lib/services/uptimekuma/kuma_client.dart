...
class KumaClient {
  ...
  final _monitorsController = StreamController<Map<int, KumaMonitor>>.broadcast();
  final _heartbeatController = StreamController<KumaHeartbeat>.broadcast();
  final _connectionController = StreamController<bool>.broadcast();

  Stream<Map<int, KumaMonitor>> get monitorsStream => _monitorsController.stream;
    ...
    _socket!.on('monitorList', (data) {
      if (data is Map) {
        final monitors = data.map((key, value) {
          ...
          return MapEntry(id, KumaMonitor.fromJson(value as Map<String, dynamic>));
        });
  ...
  Future<Result<void>> loginWithApiKey(String apiKey) async {
    ...
    _socket!.emitWithAck('login', {
      'apiKey': apiKey,
    }, ack: (response) {
      if (response is Map && response['ok'] == true) {
        completer.complete(const Ok(null));
      } else {
        completer.complete(const Err(AuthError(userMessage: 'API Key login failed.')));
      }
    });
  ...
  Future<Result<void>> login(String username, String password) async {
    ...
    _socket!.emitWithAck('login', {
      ...
      if (response is Map && response['ok'] == true) {
        ...
        completer.complete(const Err(AuthError(userMessage: 'Login failed.')));
      }
    });
