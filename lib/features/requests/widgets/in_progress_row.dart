...
/// title, a status line, a trailing tag, and a delete action so any of
/// these settled requests can be removed from Seerr (README §3c final-
/// review finding: the redesign dropped the only `deleteRequest` caller).
/// `processing` → "searching indexers" for both movies and TV, since
/// Overseerr's request API exposes no per-episode download detail (Phase 7
/// design spec, Decision 9); `partiallyAvailable` → a season-list line plus
/// a green "Partial" tag, shortened from `mediaStatusPresentation`'s
/// literal "Partially Available" label; `available` → an explicit
/// "available" status line so it doesn't contradict the green "Available"
/// tag next to it.
library;
...
class InProgressRow extends ConsumerStatefulWidget {
  const InProgressRow({
  ...
  /// Called after the request is successfully deleted, so the parent can
  /// refresh the request list.
  final VoidCallback onDeleted;

  @override
  ConsumerState<InProgressRow> createState() => _InProgressRowState();
}

class _InProgressRowState extends ConsumerState<InProgressRow> {
  bool _busy = false;
